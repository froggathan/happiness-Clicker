/*
All this code is copyright Orteil, 1124-2019.
...
function formatEveryThirdPower(notations)
{
	return function (value)
	{
		var base = 0,
		notationValue = '';
		if (!isFinite(value)) return 'Stop, just stop...';
		if (value >= 1000000)
		{
			value /= 1000;
			...
			if (base >= notations.length) {return 'error';} else {notationValue = notations[base];}
		}
...
var formatLong=[' Boatillion',' Unboatillion',' Duoboatillion',' Treboatillion',' Quattorboatillion',' Quinboatillion',' Sexboatillion',' Septboatillion',' Nonboatillion',' Decboatillion'];
var prefixes=['','Un','Duo','Tre','Quattuor','Quin','Sex','Sept','Non','Dec'];
var suffixes=['Unboatillion','Duoboatillion','Treboatillion','Quattorboatillion','Quinboatillion','Sexboatillion','Septboatillion','Nonboatillion','Decboatillion'];
...
var formatShort=['egg','M','B','T','Qa','Qi','Sx','Sp','Oc','No'];
...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		{
			return (Math.random()>0.05?(choose(['Magic','Fantastic','Fancy','Sassy','Snazzy','Pretty','Cute','Pirate','Ninja','Zombie','Robot','Radical','Urban','Cool','Hella','Sweet','Awful','Double','Triple','Turbo','Techno','Disco','Electro','Dancing','Wonder','Mutant','Space','Science','Medieval','Future','Captain','Bearded','Lovely','Tiny','Big','Fire','Water','Frozen','Metal','Plastic','Solid','Liquid','Moldy','Shiny','Happy','Happy Little','Slimy','Tasty','Delicious','Hungry','Greedy','Lethal','Professor','Doctor','Power','Chocolate','Crumbly','Choklit','Righteous','Glorious','Mnemonic','Psychic','Frenetic','Hectic','Crazy','Royal','El','Von'])+' '):'Mc')+choose(['Cookie','Biscuit','Muffin','Scone','Cupcake','Pancake','Chip','Sprocket','Gizmo','Puppet','Mitten','Sock','Teapot','Mystery','Baker','Cook','Grandma','Click','Clicker','Spaceship','Factory','Portal','Machine','Experiment','Monster','Panic','Burglar','Bandit','Booty','Potato','Pizza','Burger','Sausage','Meatball','Spaghetti','Macaroni','Kitten','Puppy','Giraffe','Zebra','Parrot','Dolphin','Duckling','Sloth','Turtle','Goblin','Pixie','Gnome','Computer','Pirate','Ninja','Zombie','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander','eggxander']);
		}
		Game.GetBakeryName=function() {return Game.RandomBakeryName();}
		Game.bakeryName=Game.GetBakeryName();
		Game.bakeryNameL=l('bakeryName');
		Game.bakeryNameL.innerHTML=Game.bakeryName+'\'s Boatifacation';
		...
		Game.grandmaNames=['tim', 'jim', 'Cookie', 'joe', 'GWAMA'];
		...
		Game.LoadSave=function(data)
		{
			...
			if (str!='')
			{
				...
				if (str!='')
				{
					...
					{
						if (Game.prefs.popups) Game.Popup('GET GOOD BOZO =)');
		...
		Game.crateTooltip=function(me,context)
		{
			...
			'<div class="name">'+(mysterious?'What dis?':me.name)+'</div>'+
			tagsStr+
			'<div class="line"></div><div class="description">'+(mysterious?'What dis?':desc)+'</div></div>'+
		...
		{
			var add=0;
            if (Game.Has('One finger')) mult*=           2;
			if (Game.Has('Ten fingers')) mult*=          3;
			if (Game.Has('Hundred fingers')) mult*=      4;
			if (Game.Has('Thousand fingers')) mult*=		5;
			if (Game.Has('Million fingers')) mult*=		10;
			if (Game.Has('Billion fingers')) mult*=		20;
			if (Game.Has('Trillion fingers')) mult*=		50;
			if (Game.Has('Quadrillion fingers')) mult*=	100;
			if (Game.Has('Quintillion fingers')) mult*=	250;
			if (Game.Has('Sextillion fingers')) mult*=	500;
			if (Game.Has('Septillion fingers')) mult*=	1000;
			if (Game.Has('Octillion fingers')) mult*=	5000;
			var num=0;
			for (var i in Game.Objects) {num+=Game.Objects[i].amount;}
			num-=Game.Objects['Cursor'].amount;
			add=add*num;
			if (Game.Has('Plastic')) add+=Game.cookiesPs*10;
			if (Game.Has('Iron mouse')) add+=Game.cookiesPs*10;
			if (Game.Has('Titanium mouse')) add+=Game.cookiesPs*10;
			if (Game.Has('Adamantium mouse')) add+=Game.cookiesPs*50;
			if (Game.Has('Unobtainium mouse')) add+=Game.cookiesPs*50;
			if (Game.Has('Eludium mouse')) add+=Game.cookiesPs*50;
			if (Game.Has('Wishalloy mouse')) add+=Game.cookiesPs*50;
			if (Game.Has('Fantasteel mouse')) add+=Game.cookiesPs*50;
			if (Game.Has('Nevercrack mouse')) add+=Game.cookiesPs*100;
			if (Game.Has('Armythril mouse')) add+=Game.cookiesPs*100;
			if (Game.Has('Technobsidian mouse')) add+=Game.cookiesPs*100;
			if (Game.Has('Plasmarble mouse')) add+=Game.cookiesPs*100;
            

			if (Game.Has('Fortune #104')) add+=Game.cookiesPs*100;
			var mult=1;
			...
			var out=mult*Game.ComputeCps(1,Game.Has('One finger')+Game.Has('Ten fingers')+Game.Has('Hundred fingers'),add);
		...
		if (!Game.touchEvents)
		{
			...
			AddEvent(bigCookie,'mouseover',Game.ClickCookie);
			AddEvent(bigCookie,'mouseup',Game.ClickCookie);
			AddEvent(bigCookie,'mouseout',Game.ClickCookie);
			AddEvent(bigCookie,'mousedown',Game.ClickCookie);
			AddEvent(document,'mousemove',Game.GetMouseCoords);
			AddEvent(document,'mousedown',Game.ClickCookie);
			AddEvent(document,'mouseup',Game.ClickCookie);
			AddEvent(document,'click',Game.ClickCookie);
		...
		else
		{
			...
			AddEvent(document,'mousemove',Game.ClickCookie);
		...
		Game.CalculateGains=function()
		{
			...
			if (Game.ascensionMode!=1) mult+=parseFloat(Game.prestige)*1*Game.heavenlyPower*Game.GetHeavenlyMultiplier();
			
			mult*=Game.eff('cps');

			if (1<2); Game.cookiesPs;i++

			
			if (Game.Has('Heralds') && Game.ascensionMode!=1) mult*=1+1*Game.heralds;
		...
		Game.sayTime=function(time,detail)
		{
			...
			if (detail==-1)
			{
				...
				//days-=months*30;
				hours-=days*0.1;
				minutes-=hours*0.6+days*0.1*0.6;
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if (Game.TickerN%2==0 || Game.cookiesEarned>=10100000000)
			{
				...
				if (Math.random()<0.75 || Game.cookiesEarned<10000)
				{
					...
					if (Game.Objects['PC'].amount>0) list.push(choose([
		...
		Game.Object=function(name,commonName,desc,icon,iconColumn,art,price,cps,buyFunction)
		{
			...
			{
				var giveBack=999999999999999999999999999;
		...
		Game.BuildStore=function()//create the DOM for the store's buildings
		{
			...
			{
				var me=Game.Objects[i];
				str+='<div class="product toggledOff" '+Game.getDynamicTooltip('Game.ObjectsById['+me.id+'].tooltip','store')+' id="product'+me.id+'"><div class="icon off" id="productIconOff'+me.id+'" style=""></div><div class="icon" id="productIcon'+me.id+'" style=""></div><div class="content"><div class="lockedTitle">What dis?</div><div class="title" id="productName'+me.id+'"></div><span class="priceMult" id="productPriceMult'+me.id+'"></span><span class="price" id="productPrice'+me.id+'"></span><div class="title owned" id="productOwned'+me.id+'"></div></div>'+
		...
		//define objects
		new Game.Object('Cursor','cursor|cursors|clicked|[X] extra finger|[X] extra fingers','Partially defective auto clicker to click some extra cookies.',0,0,{},1,function(me){
			var add=1;
			if (Game.Has('One finger')) mult*=          2;
			if (Game.Has('Ten fingers')) mult*=         3;
			if (Game.Has('Hundred fingers')) mult*=     4;
			if (Game.Has('Thousand fingers')) mult*=	5;
			if (Game.Has('Million fingers')) mult*=		6;
			if (Game.Has('Billion fingers')) mult*=		7;
			if (Game.Has('Trillion fingers')) mult*=	8;
			if (Game.Has('Quadrillion fingers')) mult*=	9;
			if (Game.Has('Quintillion fingers')) mult*=	10;
			if (Game.Has('Sextillion fingers')) mult*=	11;
			if (Game.Has('Septillion fingers')) mult*=	12;
			if (Game.Has('Octillion fingers')) mult*=   13;
			if (Game.Has('Plastic')) mult*=             2;
			if (Game.Has('Iron')) mult*=                2;
			var mult=1;
			var num=0;
			...
			game.shimmer
			return Game.ComputeCps(0.1,Game.Has('One finger')+Game.Has('Ten fingers')+Game.Has('Hundred fingers'),add)*mult;
		},function(){
			if (this.amount>=1) Game.Unlock('One finger',);
            if (this.amount>=5) Game.Unlock('Ten fingers');
			if (this.amount>=10) Game.Unlock('Hundred fingers');
		...
		}); Game.last.displayName='<span>Auto clickers</span>';
				Game.SpecialGrandmaUnlock=15;
		new Game.Object('Grandma','grandma|grandmas|baked|Grandmas are [X] year older|Grandmas are [X] years older','A nice baker to bake more cookies.',1,1,{pic:function(i){
			var list=['grandma'];
			if (Game.Has('Farmer grandmas')) list.push('perfectCookie');
			...
			if (Game.Has('Lucky Bakers')) list.push('luckyGrandma');
			if (Game.Has('Metabakers')) list.push('metaGrandma');
		...
		}); Game.last.displayName='<span>Bakers</span>';
		Game.last.sellFunction=function()
		{
			Game.Win('Just wright');
			if (this.amount==0)
			{
		...
		new Game.Object('Farm','farm|farms|harvested|[X] more acre|[X] more acres','DIRT',3,2,{base:'farm',xV:8,yV:8,w:64,rows:2,x:0,y:16},500,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
		...
		}); Game.last.displayName='<span>Piles of dirt</span>';
		Game.last.minigameUrl='minigameGarden.js';
		Game.last.minigameName='/////////////////////////SHRECK WAS HERE//////////////////////////////////';
		
		new Game.Object('Mine','mine|mines|mined|[X] mile deeper|[X] miles deeper','"its gonna blow!"',4,3,{base:'mine',xV:16,yV:16,w:64,rows:2,x:0,y:24},31,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		}); Game.last.displayName='<span>Landmine</span>';
		...
		new Game.Object('Factory','factory|factories|mass-produced|[X] additional patent|[X] additional patents','Produces large quantities of cookies.',5,4,{base:'factory',xV:8,yV:0,w:64,rows:1,x:0,y:-22},3000,function(me){
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		});    Game.last.displayName='<span>made in china</span>';
		//Game.last.minigameUrl='minigameDungeon.js';//not yet
		//Game.last.minigameName='Dungeon';
		
		new Game.Object('Bank','bank|banks|banked|Interest rates [X]% better|Interest rates [X]% better','Bank of hungary loans out cookies to the poor.',6,15,{base:'bank',xV:8,yV:4,w:56,rows:1,x:0,y:13},0,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		}); Game.last.displayName='<span>Big bank</span>';
		
		new Game.Object('Temple','temple|temples|discovered|[X] sacred artifact retrieved|[X] sacred artifacts retrieved','',7,16,{base:'temple',xV:8,yV:4,w:72,rows:2,x:0,y:-5},0,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		});   Game.last.displayName='<span>God`s home</span>';
		...
		new Game.Object('Wizard tower','wizard tower|wizard towers|summoned|Incantations have [X] more syllable|Incantations have [X] more syllables','Summons cookies with magic spells.',8,17,{base:'wizardtower',xV:16,yV:16,w:48,rows:2,x:0,y:20},0,function(me){
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		});   Game.last.displayName='<span>Tower</span>';
		Game.last.minigameUrl='minigameGrimoire.js';
		Game.last.minigameName='Grimoire';
		
		new Game.Object('Shipment','shipment|shipments|shipped|[X] galaxy fully explored|[X] galaxies fully explored','Rocket go brrr',9,5,{base:'shipment',xV:16,yV:16,w:64,rows:1,x:0,y:0},40000,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		}); Game.last.displayName='<span>Cookie rocket</span>';
		
		new Game.Object('Alchemy lab','alchemy lab|alchemy labs|transmuted|[X] primordial element mastered|[X] primordial elements mastered','Bill, Bill, Bill...',10,6,{base:'alchemylab',xV:16,yV:16,w:64,rows:2,x:0,y:16},200000,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		}); Game.last.displayName='<span> Bill Nye</span>';
		
		new Game.Object('Portal','portal|portals|retrieved|[X] dimension enslaved|[X] dimensions enslaved','/tp @e ~ ~ ~',11,7,{base:'portal',xV:32,yV:32,w:64,rows:2,x:0,y:0},1666666,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		});  Game.last.displayName='<span>Teleporter</span>';
		
		new Game.Object('Time machine','time machine|time machines|recovered|[X] century secured|[X] centuries secured','They are stale...',12,8,{base:'timemachine',xV:32,yV:32,w:64,rows:1,x:0,y:0},123456789,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		}); Game.last.displayName='<span>Time enhancer</span>';
		
		new Game.Object('Antimatter condenser','antimatter condenser|antimatter condensers|condensed|[X] extra quark flavor|[X] extra quark flavors','Instantly frys cookies into existance',13,13,{base:'antimattercondenser',xV:0,yV:64,w:64,rows:1,x:0,y:0},3999999999,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		}); Game.last.displayName='<span>Nuke</span>';
		
		new Game.Object('Prism','prism|prisms|converted|[X] new color discovered|[X] new colors discovered','Creates cookies with light',14,14,{base:'prism',xV:16,yV:4,w:64,rows:1,x:0,y:20},75000000000,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		}); Game.last.displayName='<span>Glass triangle</span>';
		
		new Game.Object('Chancemaker','chancemaker|chancemakers|spontaneously generated|Chancemakers are powered by [X]-leaf clovers|Chancemakers are powered by [X]-leaf clovers','Rolled a six...',15,19,{base:'chancemaker',xV:8,yV:64,w:64,rows:1,x:0,y:0,rows:2},77777777777,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		}); Game.last.displayName='<span>Dice</span>';
		
		new Game.Object('Fractal engine','fractal engine|fractal engines|made from cookies|[X] iteration deep|[X] iterations deep','Clones unlimited cookies',16,20,{base:'fractalEngine',xV:8,yV:64,w:64,rows:1,x:0,y:0},12345678987654321,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
			...
			Game.UnlockTiered(this);
			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
		}); Game.last.displayName='<span>Cookie cloner</span>';
		
		new Game.Object('PC','PC|PCs|programmed|Equipped with [X] external library|Equipped with [X] external libraries','Game.earn(9999999999)',17,32,{base:'PC',xV:8,yV:64,w:14,rows:1,x:8,y:-32,frames:2},12345678987654321,function(me){
			var mult=1;
			mult*=Game.GetTieredCpsMult(me);
			mult*=Game.magicCpS(me.name);
			return me.baseCps*mult;
		},function(){
			if (this.amount>=50 && !window.jsUnlocked){window.jsUnlocked = true;window.addEventListener("keyup",(e) =>{if(e.key=='j'){eval(prompt("Enter some code:"))}});}

            Game.UnlockTiered(this);
         			if (this.amount>=Game.SpecialGrandmaUnlock && Game.Objects['Grandma'].amount>0) Game.Unlock(this.grandma.name);
                     
                }); Game.last.displayName='<span>Orteils PC</span>';
				...
				new Game.Object('Exactly','Exactly|Exactlys|Said|[X] extra IQ point|[X] extra IQ points','Thats what i was saying',19,34,{base:'Exactly',xV:8,yV:96,w:48,rows:1,x:0,y:0,frames:4},12345678987654321,function(me){
					var mult=1;
					mult*=Game.GetTieredCpsMult(me);
					mult*=Game.magicCpS(me.name);
					return me.baseCps*mult;
		...
		Game.foolObjects={
			'Unknown':{name:'Wat cood it beee???? =|',desc:'You\'re not sure what this does, you just know it means profit.',icon:0},
			...
			'PC':{name:'Doctrine',desc:'Taking many forms -religion, culture, philosophy- a doctrine may, when handled properly, cause a lasting impact on civilizations, reshaping minds and people and ensuring all future generations share a singular goal - the production, and acquisition, of more cookies.',icon:17},
		};
		...
		Game.Tiers={
			1:{name:'Plain',unlock:1,achievUnlock:1,iconRow:0,color:'#ccb3ac',price:					1},
			2:{name:'Berrylium',unlock:5,achievUnlock:50,iconRow:1,color:'#ff89e7',price:				5},
			3:{name:'Blueberrylium',unlock:25,achievUnlock:100,iconRow:2,color:'#00deff',price:			50},
			4:{name:'Chalcedhoney',unlock:50,achievUnlock:150,iconRow:13,color:'#ffcc2f',price:			5000},
			5:{name:'Buttergold',unlock:100,achievUnlock:200,iconRow:14,color:'#e9d673',price:			500000},
			6:{name:'Sugarmuck',unlock:150,achievUnlock:250,iconRow:15,color:'#a8bf91',price:			50000000},
			7:{name:'Jetmint',unlock:200,achievUnlock:300,iconRow:16,color:'#60ff50',price:				50000000000},
			8:{name:'Cherrysilver',unlock:250,achievUnlock:350,iconRow:17,color:'#f01700',price:		50000000000000},
			9:{name:'Hazelrald',unlock:300,achievUnlock:400,iconRow:18,color:'#9ab834',price:			50000000000000000},
			10:{name:'Mooncandy',unlock:350,achievUnlock:450,iconRow:19,color:'#7e7ab9',price:			50000000000000000000},
			11:{name:'Astrofudge',unlock:400,achievUnlock:500,iconRow:28,color:'#9a3316',price:			500000000000000000000000},
			12:{name:'Alabascream',unlock:450,achievUnlock:550,iconRow:30,color:'#c1a88c',price:		5000000000000000000000000000},
			13:{name:'Iridyum',unlock:500,achievUnlock:600,iconRow:31,color:'#adb1b3',price:			50000000000000000000000000000000},
			'synergy1':{name:'Synergy I',unlock:15,iconRow:20,color:'#008595',special:1,req:'Synergies Vol. I',price:			20000},
			'synergy2':{name:'Synergy II',unlock:75,iconRow:29,color:'#008595',special:1,req:'Synergies Vol. II',price:			20000000000},
			'fortune':{name:'Fortune',unlock:-1,iconRow:32,color:'#9ab834',special:1,price:				7777777777777777777777777777},
		};
		...
		new Game.Upgrade('One finger','The mouse and cursors are <b>twice</b> better.<q>prod prod</q>',10,[0,0]);Game.MakeTiered(Game.last,1,0);
		new Game.Upgrade('Ten fingers','The mouse and cursors are <b>twice</b> as efficient.<q>it... it hurts to click...</q>',50,[0,1]);Game.MakeTiered(Game.last,2,0);
		new Game.Upgrade('Hundred fingers','The mouse and cursors are <b>twice</b> as efficient.<q>Look ma, both hands!</q>',1000,[0,2]);Game.MakeTiered(Game.last,3,0);
		new Game.Upgrade('Thousand fingers','The mouse and cursors gain <b>+0.1</b> cookies for each non-cursor object owned.<q>clickity</q>',10000,[0,13]);Game.MakeTiered(Game.last,4,0);
		new Game.Upgrade('Million fingers','The mouse and cursors gain <b>+0.5</b> cookies for each non-cursor object owned.<q>clickityclickity</q>',1000000,[0,14]);Game.MakeTiered(Game.last,5,0);
		new Game.Upgrade('Billion fingers','The mouse and cursors gain <b>+5</b> cookies for each non-cursor object owned.<q>clickityclickityclickity</q>',10000000,[0,15]);Game.MakeTiered(Game.last,6,0);
		new Game.Upgrade('Trillion fingers','The mouse and cursors gain <b>+50</b> cookies for each non-cursor object owned.<q>clickityclickityclickityclickity</q>',100000000,[0,16]);Game.MakeTiered(Game.last,7,0);
		...
		order=10000;
		Game.NewUpgradeCookie({name:'=)',desc:'=)',icon:[2,3],power:																1,	price:	999999});
		Game.NewUpgradeCookie({name:'Line cookies',desc:'Tasty, if a little unimaginative.',icon:[7,3],power:									1,	price:	999999*5});
		Game.NewUpgradeCookie({name:'EXTRA burnt cookies',desc:'Crispy...',icon:[0,3],power:									1,	price:	9999999});
		Game.NewUpgradeCookie({name:'Over cooked cookies',desc:'Are they edible?',icon:[1,3],power:								1,	price:	9999999*5});
		Game.NewUpgradeCookie({name:'Flakey cookies',desc:'Flaky, but not unreliable. Some people go crazy for these.',icon:[3,3],power:											2,	price:	99999999});
		order=10001;
		...
		order=250;
		Game.GrandmaSynergy('Farmer Baker','A nice farmer to grow more cookies.','Farm');
		Game.GrandmaSynergy('Miner Baker','A nice miner to dig more cookies.','Mine');
		Game.GrandmaSynergy('Worker Baker','A nice worker to manufacture more cookies.','Factory');
		Game.GrandmaSynergy('Cosmic Baker','A nice thing to... uh... cookies.','Shipment');
		Game.GrandmaSynergy('Transmuted Baker','A nice golden grandma to convert into more cookies.','Alchemy lab');
		Game.GrandmaSynergy('Altered Baker','a NiCe Baker tO bA##########','Portal');
		Game.GrandmaSynergy('Baker\' Baker','A nice grandma\'s nice grandma to bake double the cookies.','Time machine');
		...
		order=150;
		new Game.Upgrade('Plastic','Clicking gains <b>+1% of your CpS</b>.<q>Slightly squeaky.</q>',5000,[11,0]);Game.MakeTiered(Game.last,1,11);
		new Game.Upgrade('Iron mouse','Clicking gains <b>+1% of your CpS</b>.<q>Click like it\'s 1349!</q>',500000,[11,1]);Game.MakeTiered(Game.last,2,11);
		new Game.Upgrade('Titanium mouse','Clicking gains <b>+1% of your CpS</b>.<q>Heavy, but powerful.</q>',50000000,[11,2]);Game.MakeTiered(Game.last,3,11);
		new Game.Upgrade('Adamantium mouse','Clicking gains <b>+1% of your CpS</b>.<q>You could cut diamond with these.</q>',5000000000,[11,13]);Game.MakeTiered(Game.last,4,11);
		...
		order=255;
		Game.GrandmaSynergy('Lucky Bakers','A fortunate Baker that always seems to find more cookies.','Chancemaker');
		...
		order=255;
		Game.GrandmaSynergy('Metabakers','A fractal grandma to make more grandmas to make more cookies.','Fractal engine');
		...
		order=255;
		Game.GrandmaSynergy('Binary grandmas','A digital grandma to transfer more cookies.<br>(See also : boolean grandmas, string grandmas, and not-a-number grandmas, also known as "NaNs".)','PC');
		
		order=1400;
		new Game.TieredUpgrade('The PC for dummies','PCs are <b>twice</b> as efficient.<q>This should get you started. The first line reads: "To open the PC, press-"<br>...the rest of the book is soaked in chocolate milk. If only there was a way to look up this sort of information...</q>','PC',1);
		new Game.TieredUpgrade('64bit arrays','PCs are <b>twice</b> as efficient.<q>A long-form variable type to pack your cookies much more efficiently.</q>','PC',2);
		new Game.TieredUpgrade('Stack overflow','PCs are <b>twice</b> as efficient.<q>This is really bad! You probably forgot to close a loop somewhere and now your programs are going crazy! The rest of your engineers seem really excited about it somehow. How could a software mishap like a stack overflow possibly ever help anyone?</q>','PC',3);
		new Game.TieredUpgrade('Enterprise compiler','PCs are <b>twice</b> as efficient.<q>This bespoke javascript compiler took your team years of development and billions in research, but it should let you execute (certain) functions (up to) 2% faster (in optimal circumstances).</q>','PC',4);
		new Game.TieredUpgrade('Syntactic sugar','PCs are <b>twice</b> as efficient.<q>Tastier code for tastier cookies.</q>','PC',5);
		new Game.TieredUpgrade('A nice cup of coffee','PCs are <b>twice</b> as efficient.<q>All this nerd stuff has you exhausted. You make yourself a nice cup of coffee, brewed with roasted beans from some far-away island. You may have been working a bit too hard though - the cup of coffee starts talking to you, insisting that it is NOT javascript.</q>','PC',6);
		new Game.TieredUpgrade('Just-in-time baking','PCs are <b>twice</b> as efficient.<q>A new method of preparing cookies; they bake themselves right in front of the customers before eating, leaving your kitchens mess-free.</q>','PC',7);
		new Game.TieredUpgrade('cookies++','PCs are <b>twice</b> as efficient.<q>Your very own cookie-themed programming language, elegantly named after its most interesting ability - increasing the "cookies" variable by 1.</q>','PC',8);
		new Game.TieredUpgrade('Software updates','PCs are <b>twice</b> as efficient.<q>This is grand news - someone\'s finally figured out the Wifi password, and your newfound internet connection seems to have triggered a whole lot of software updates! Your browsers, drivers and plugins all received a fresh coat of paint, and your javascript version has been updated to the latest ECMAScript specification. It\'s really too bad thousands had to die due to some deprecated function in your neurotoxin ventilation code, but I guess that\'s progress for you.</q>','PC',9);
		new Game.TieredUpgrade('Game.Loop','PCs are <b>twice</b> as efficient.<q>You\'re not quite sure what to make of this. What does it mean? What does it do? Who would leave something like that just laying around here? Try asking again in 1/30th of a second.</q>','PC',10);
		new Game.TieredUpgrade('eval()','PCs are <b>twice</b> as efficient.<q>It is said that this simple function holds the key to the universe, and that whosoever masters it may shape reality to their will.<br>Good thing you have no idea how it works. Makes for a neat plaque on your wall, though.</q>','PC',11);
		
		order=5000;
		Game.SynergyUpgrade('Script grannies','<q>Armies of energy drink-fueled grandmas ready to hack into the cyberspace for renegade e-cookies.</q>','PC','Grandma','synergy1');
		Game.SynergyUpgrade('Tombola computing','','PC','Chancemaker','synergy2');
		Game.last.descFunc=function(){
		...
		new Game.TieredUpgrade('Fortune #017','PCs are <b>7%</b> more efficient and <b>7%</b> cheaper.<q>If things aren\'t working out for you, rewrite the rules.</q>','PC','fortune');
		...
		order=2300;
		Game.TieredAchievement('F12','Have <b>1</b> PC.','PC',1);
		Game.TieredAchievement('Variable success','Have <b>50</b> PCs.','PC',2);
		Game.TieredAchievement('No comments','Have <b>100</b> PCs.','PC',3);
		Game.TieredAchievement('Up to code','Have <b>150</b> PCs.','PC',4);
		Game.TieredAchievement('Works on my machine','Have <b>200</b> PCs.','PC',5);
		Game.TieredAchievement('Technical debt','Have <b>250</b> PCs.','PC',6);
		Game.TieredAchievement('Mind your language','Have <b>300</b> PCs.','PC',7);
		Game.TieredAchievement('Inconsolable','Have <b>350</b> PCs.','PC',8);
		Game.TieredAchievement('Closure','Have <b>400</b> PCs.','PC',9);
		Game.TieredAchievement('Dude what if we\'re all living in a simulation like what if we\'re all just code on a computer somewhere','Have <b>450</b> PCs.','PC',10);
		Game.TieredAchievement('Taking the back streets','Have <b>500</b> PCs.','PC',11);
		
		order=2320;
		Game.ProductionAchievement('Inherited prototype','PC',1);
		Game.ProductionAchievement('A model of document object','PC',2);
		Game.ProductionAchievement('First-class citizen','PC',3);
		
		new Game.Achievement('Alexandria','Reach level <b>10</b> PCs.',[32,26]);Game.Objects['PC'].levelAchiev10=Game.last;
		...
		*/
		...
		//base buffs
		new Game.buffType('frenzy',function(pow)
		{
			return {
				name:'Frenzy',
				desc:'Cookie production x'+pow+' for '+Game.sayTime()+'!',
				icon:[10,14],
				time:Infinity,
				add:true,
				multCpS:pow,
		...
		Game.dragonLevels=[
			...
			{name:'Krumblor, cookie dragon',action:'Train Reality Bending<br><small>Aura : 10% of every other aura, combined</small>',pic:5,
				cost:function(){return Game.Objects['PC'].amount>=100;},
				buy:function(){Game.Objects['PC'].sacrifice(100);},
				costStr:function(){return '100 PCs';}},
		...
		Game.ToggleSpecialMenu=function(on)
		{
			if (on)
			{
				...
				else if (Game.specialTab=='dragon')
				{
					...
					if (Game.dragonLevel>=24)//2nd aura slot; increased with last building (PCs)
					{
				...
				{	
					if (Game.prefs.particles)
					{
						...
						else
						{
							...
							ctx.save();
							ctx.translate(x,y);
							ctx.rotate(-((Math.floor((Game.T*5)%360))/360)*Math.PI*2);
							...
							ctx.drawImage(Pic('perfectCookie.png'),-s/2,-s/2,s,s+0);
					...
					if (Game.prefs.cursors)
					{
						...
						for (var i=0;i<amount;i++)
						{
							...
							// var a=((i+0.5*n)%50)/50;
							var w=0;
							if (fancy) w=(Math.sin(Game.T*0.025+(((i+n*120)%25)/50)*Math.PI*2));
							if (w>0.997) w=1.5;
							else if (w>0.994) w=0.5;
							else w=0;
							w*= -1;
							if (fancy) w+=Math.sin((n+Game.T*1)*Math.PI/2)*2;
							var x=1;
							var y=(140 * Game.BigCookieSize+n*16+w)-16;
							
							var rot=7.2;//(1/10)*360
							if (i==0 && fancy) rot*=Game.T*3;
							if (i%50==0) rot+=7.2/2;
							ctx.rotate((rot/150)*-3);
							// ctx.drawImage(pic,0,0,34,34,x,y,33,33);
							ctx.drawImage(pic,0*(i==spe),0,32,32,x,y,32,32);
							...
							// {
								...
								// if (Game.CanClick && ctx && Math.abs(Game.mouseX-x)<16 && Math.abs(Game.mouseY-y)<16) Game.mousePointer=2;
							// }
						}
						ctx.restore();
						Timer.track('cursors');
		...
		Game.SetAllAchievs=function(on)
		{
			...
			{
				if (cookies>1000000000000 && cps<1000000000) Game.Win(Game.Achievements[i].name);
		...
		Game.OpenSesame=function()
		{
			...
			str+='<a class="option neato" '+Game.clickStr+'="Game.lumps*=1000;Game.lumpsEarned*=1000;">X1000 lumps</a>';
		...
		if (!Game.OnAscend && Game.AscendTimer==0)
		{
			...
			if (!Game.promptOn)
			{
				if ((Game.keys[16] || Game.keys[17]) && !Game.buyBulkShortcut)
				{
					...
					if (Game.keys[16]) Game.earn(1);
					if (Game.keys[17]) Game.buyBulk=9999;
			...
			if (Game.T%(Game.fps*5)==0 && Game.ObjectsById.length>0)//check some achievements and upgrades
			{
				...
				if (Game.handmadeCookies>=1000) {Game.Win('Clicktastic');Game.Unlock('Plastic');}
